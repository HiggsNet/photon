...
- [ ] 继续删除只有一个调用方的 wrapper、重复 clone/DTO builder 和 legacy 测试准备；测试跟随实际 owner 迁移。
  ...
  - [x] 删除 `internal/inspect/http` 中 routes/peers/status/zones 的纯 type alias 与函数变量转发；Observer 和文本测试直接使用 canonical `internal/inspect` DTO，HTTP 目录只保留确有独立 wire shape 的 links/health/BIRD。
